...
class Engine {
  ...
  _bindEvents() {
    ...
    document.getElementById('intro-overlay').addEventListener('click', () => this.nextIntro())
  ...
  render(now) {
    ...
    if (this.scene) {
      ...
      else { ctx.fillStyle = '#0a0505'; ctx.fillRect(0, 0, this.W, this.H) }

      const el = document.getElementById('hud-watching')
      if (this.scene==='tower'||this.scene==='tunnel'){
        el.classList.add('show')
      }else if(['corridor_1','corridor_3','corridor_5','cellar','church','graveyard'].includes(this.scene)){
        if(Math.sin(now*0.0008)>0.85)el.classList.add('show')
  ...
  hitTest(obj) {
    ...
    const mx = this.canvas.width * this.mouseX / this.W
    const my = this.canvas.height * this.mouseY / this.H
    return h
  }
  findHit(x, y) {
